...
**Date:** April 18, 2026
**Version:** V45 (Level 5 guideStone: `ludospring_guidestone` binary uses primalSpring composition API (`CompositionContext`, `validate_parity`, `validate_liveness`) — capability-routed IPC to barraCuda (Fitts, Hick, sigmoid, log2, mean, std_dev, Perlin, rng, tensor). Inherits primalSpring base certification (6 layers). Five-layer validation: Python→Rust→IPC composition→primal proof→guideStone. **790+** workspace tests (0 failed). Harvested to plasmidBin v0.10.0.)
**Spring alignment table:** The ludoSpring row in sibling `../primalSpring/wateringHole/NUCLEUS_SPRING_ALIGNMENT.md` uses the same workspace test total as this README (**790+** as of V45); if they diverge, treat this README and `cargo test --workspace` as canonical.
