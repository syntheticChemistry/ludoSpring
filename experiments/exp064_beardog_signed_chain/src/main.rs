...
use signed_chain::{
    Ed25519KeyPair, SignedProvenanceChain,
    sign_vertex_ipc, verify_signature_ipc, hash_content_ipc,
};
...
fn validate_key_pair() -> Vec<ValidationResult> {
    ...
    results.push(ValidationResult::check(
        EXP, "keypair_public_key_32_bytes",
        kp.public_key.len() as f64, 32.0, 0.0,
    ));

    let message = b"hello provenance chain";
    let sig = kp.sign(message);
    results.push(ValidationResult::check(
        EXP, "signature_64_bytes",
        sig.len() as f64, 64.0, 0.0,
    ));

    let valid = Ed25519KeyPair::verify(&kp.public_key, message, &sig);
    results.push(ValidationResult::check(
        EXP, "signature_verifies_correctly",
        bool_f64(valid), 1.0, 0.0,
    ));
    ...
    results.push(ValidationResult::check(
        EXP, "tampered_message_rejected",
        bool_f64(!invalid), 1.0, 0.0,
    ));
    ...
    results.push(ValidationResult::check(
        EXP, "tampered_signature_rejected",
        bool_f64(!invalid2), 1.0, 0.0,
    ));

    let kp2 = Ed25519KeyPair::from_seed("bob_key_seed_2026");
    let wrong_key = Ed25519KeyPair::verify(&kp2.public_key, message, &sig);
    results.push(ValidationResult::check(
        EXP, "wrong_key_rejected",
        bool_f64(!wrong_key), 1.0, 0.0,
    ));

    let kp_same = Ed25519KeyPair::from_seed("alice_key_seed_2026");
    results.push(ValidationResult::check(
        EXP, "deterministic_keypair",
        bool_f64(kp.public_key == kp_same.public_key), 1.0, 0.0,
    ));
...
fn validate_signed_lifecycle() -> Vec<ValidationResult> {
    ...
    results.push(ValidationResult::check(
        EXP, "signed_mint_cert_created",
        bool_f64(chain.cert_manager.get_certificate(&cert_id).is_some()), 1.0, 0.0,
    ));
    results.push(ValidationResult::check(
        EXP, "signed_mint_has_signature",
        chain.signed_certs.len() as f64, 1.0, 0.0,
    ));

    chain.sign_and_append_vertex("equip_sword", alice.as_str());
    results.push(ValidationResult::check(
        EXP, "signed_vertex_appended",
        chain.signed_vertices.len() as f64, 1.0, 0.0,
    ));

    chain.sign_and_create_braid("Equipped Signed Sword", alice.as_str());
    results.push(ValidationResult::check(
        EXP, "signed_braid_created",
        chain.signed_braids.len() as f64, 1.0, 0.0,
    ));
    ...
    chain.sign_and_transfer(cert_id, &alice, &bob).expect("transfer");
    chain.sign_and_append_vertex("trade_to_bob", alice.as_str());
    chain.sign_and_create_braid("Traded sword to Bob", alice.as_str());

    results.push(ValidationResult::check(
        EXP, "chain_total_vertices",
        chain.signed_vertices.len() as f64, 4.0, 0.0,
    ));
    results.push(ValidationResult::check(
        EXP, "chain_total_certs",
        chain.signed_certs.len() as f64, 2.0, 0.0,
    ));
    results.push(ValidationResult::check(
        EXP, "chain_total_braids",
        chain.signed_braids.len() as f64, 3.0, 0.0,
    ));

    let verification = chain.verify_chain();
    results.push(ValidationResult::check(
        EXP, "chain_all_verified",
        bool_f64(verification.is_clean()), 1.0, 0.0,
    ));
    results.push(ValidationResult::check(
        EXP, "chain_verified_count",
        verification.verified as f64, 9.0, 0.0,
    ));
    results.push(ValidationResult::check(
        EXP, "chain_zero_tampered",
        verification.tampered.len() as f64, 0.0, 0.0,
    ));
...
fn validate_tamper_detection() -> Vec<ValidationResult> {
    ...
    results.push(ValidationResult::check(
        EXP, "pre_tamper_clean",
        bool_f64(pre_tamper.is_clean()), 1.0, 0.0,
    ));

    chain.signed_vertices[1].content_hash[0] ^= 0xFF;
    let post_tamper = chain.verify_chain();
    results.push(ValidationResult::check(
        EXP, "tampered_vertex_detected",
        bool_f64(!post_tamper.is_clean()), 1.0, 0.0,
    ));
    results.push(ValidationResult::check(
        EXP, "tampered_exactly_one",
        post_tamper.tampered.len() as f64, 1.0, 0.0,
    ));
    results.push(ValidationResult::check(
        EXP, "tampered_is_vertex",
        bool_f64(post_tamper.tampered[0].starts_with("vertex:")), 1.0, 0.0,
    ));
    ...
    results.push(ValidationResult::check(
        EXP, "tampered_cert_detected",
        bool_f64(!cert_tamper.is_clean()), 1.0, 0.0,
    ));
    results.push(ValidationResult::check(
        EXP, "tampered_cert_is_cert",
        bool_f64(cert_tamper.tampered[0].starts_with("cert:")), 1.0, 0.0,
    ));
    ...
    results.push(ValidationResult::check(
        EXP, "tampered_braid_detected",
        bool_f64(!braid_tamper.is_clean()), 1.0, 0.0,
    ));
    results.push(ValidationResult::check(
        EXP, "tampered_braid_is_braid",
        bool_f64(braid_tamper.tampered[0].starts_with("braid:")), 1.0, 0.0,
    ));
...
fn validate_multi_actor() -> Vec<ValidationResult> {
    ...
    chain.sign_and_transfer(cert_id, &alice, &bob).expect("transfer A->B");
    chain.sign_and_append_vertex("bob_uses_shield", bob.as_str());
    chain.sign_and_create_braid("Bob blocks attack", bob.as_str());

    chain.sign_and_transfer(cert_id, &bob, &carol).expect("transfer B->C");
    chain.sign_and_append_vertex("carol_enchants", carol.as_str());
    chain.sign_and_create_braid("Carol enchants shield", carol.as_str());

    results.push(ValidationResult::check(
        EXP, "multi_actor_3_transfers",
        chain.signed_certs.len() as f64, 3.0, 0.0,
    ));

    let current_owner = chain.cert_manager.get_certificate(&cert_id)
        .map(|c| c.owner.as_str().to_string());
    results.push(ValidationResult::check(
        EXP, "multi_actor_carol_owns",
        bool_f64(current_owner.as_deref() == Some("did:key:carol_multi")), 1.0, 0.0,
    ));

    let verification = chain.verify_chain();
    results.push(ValidationResult::check(
        EXP, "multi_actor_chain_clean",
        bool_f64(verification.is_clean()), 1.0, 0.0,
    ));
    results.push(ValidationResult::check(
        EXP, "multi_actor_total_items",
        verification.total_items as f64, 9.0, 0.0,
    ));
...
fn validate_beardog_ipc() -> Vec<ValidationResult> {
    ...
    results.push(ValidationResult::check(
        EXP, "ipc_sign_method",
        bool_f64(sign_req["method"] == "crypto.sign_ed25519"), 1.0, 0.0,
    ));
    results.push(ValidationResult::check(
        EXP, "ipc_sign_has_params",
        bool_f64(sign_req["params"]["message"].is_string()), 1.0, 0.0,
    ));
    results.push(ValidationResult::check(
        EXP, "ipc_sign_has_key_id",
        bool_f64(sign_req["params"]["key_id"] == "alice_key_01"), 1.0, 0.0,
    ));

    let verify_req = verify_signature_ipc("msg", "sig_hex", "pubkey_hex");
    results.push(ValidationResult::check(
        EXP, "ipc_verify_method",
        bool_f64(verify_req["method"] == "crypto.verify_ed25519"), 1.0, 0.0,
    ));
    results.push(ValidationResult::check(
        EXP, "ipc_verify_has_signature",
        bool_f64(verify_req["params"]["signature"].is_string()), 1.0, 0.0,
    ));

    let hash_req = hash_content_ipc("data_to_hash");
    results.push(ValidationResult::check(
        EXP, "ipc_hash_method",
        bool_f64(hash_req["method"] == "crypto.blake3_hash"), 1.0, 0.0,
    ));
    results.push(ValidationResult::check(
        EXP, "ipc_hash_has_data",
        bool_f64(hash_req["params"]["data"] == "data_to_hash"), 1.0, 0.0,
    ));

    let sign_roundtrip: serde_json::Value = serde_json::from_str(
        &serde_json::to_string(&sign_req).unwrap()
    ).unwrap();
    results.push(ValidationResult::check(
        EXP, "ipc_json_roundtrip",
        bool_f64(sign_roundtrip["method"] == "crypto.sign_ed25519"), 1.0, 0.0,
    ));
...
fn validate_serialization() -> Vec<ValidationResult> {
    ...
    results.push(ValidationResult::check(
        EXP, "serial_sig_roundtrip",
        bool_f64(Ed25519KeyPair::verify(&kp.public_key, message, &sig_arr)), 1.0, 0.0,
    ));
    ...
    results.push(ValidationResult::check(
        EXP, "serial_pubkey_roundtrip",
        bool_f64(pk_arr == kp.public_key), 1.0, 0.0,
    ));
