...
fn generate_disorder_landscape(
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let noise_val = noise::perlin_2d(nx * 4.0 + offset, ny * 4.0 + offset);
...
fn generate_communities() -> Vec<MicrobialCommunity> {
    vec![
        MicrobialCommunity {
            ...
            diversity_h: 3.2, evenness_j: 0.85, qs_gene_density: 0.7, oxygen_level: 0.05,
        },
        MicrobialCommunity {
            name: "dysbiotic_gut".into(),
            diversity_h: 1.8, evenness_j: 0.55, qs_gene_density: 0.4, oxygen_level: 0.15,
        },
        MicrobialCommunity {
            name: "mucosal_surface".into(),
            diversity_h: 2.5, evenness_j: 0.70, qs_gene_density: 0.55, oxygen_level: 0.40,
        },
        MicrobialCommunity {
            name: "anaerobic_digester".into(),
            diversity_h: 3.8, evenness_j: 0.90, qs_gene_density: 0.85, oxygen_level: 0.0,
        },
        MicrobialCommunity {
            name: "post_antibiotic".into(),
            diversity_h: 0.8, evenness_j: 0.35, qs_gene_density: 0.2, oxygen_level: 0.20,
        },
    ]
}
...
fn simulate_exploration_session(
    ...
    for community in communities {
        ...
        let propagation = simulate_qs_propagation(
            &landscape, signal_strength, grid_size / 2, grid_size / 2,
        );
        session_data.push((
...
fn cmd_validate() {
    ...
    let all_valid = session_data.iter().all(|(_, _, p, _)| (0.0..=1.0).contains(p));
    results.push(ValidationResult::check(EXP, "all_propagation_valid", bool_f64(all_valid), 1.0, 0.0));

    // 2. High diversity dominates O2 in W model: anaerobic (H'=3.8, O2=0) > mucosal (H'=2.5, O2=0.4)
    let anaerobic = session_data.iter().find(|(n, _, _, _)| n == "anaerobic_digester");
    let mucosal = session_data.iter().find(|(n, _, _, _)| n == "mucosal_surface");
    if let (Some(a), Some(m)) = (anaerobic, mucosal) {
        println!("\n  Diversity vs O2: anaerobic W={:.2} (H'=3.8) vs mucosal W={:.2} (H'=2.5, O2=0.4)", a.1, m.1);
        results.push(ValidationResult::check(
            EXP, "diversity_dominates_o2_in_w", bool_f64(a.1 > m.1), 1.0, 0.0,
        ));
    }

    // 3. Post-antibiotic has lowest diversity
    let post_ab = session_data.iter().find(|(n, _, _, _)| n == "post_antibiotic");
    if let Some(pa) = post_ab {
        let lowest = session_data.iter().all(|(n, _, _, h)| n == "post_antibiotic" || *h > pa.3);
        results.push(ValidationResult::check(EXP, "post_antibiotic_lowest_diversity", bool_f64(lowest), 1.0, 0.0));
    }
    ...
    let det = l1.iter().zip(l2.iter())
        .all(|(r1, r2)| r1.iter().zip(r2.iter()).all(|(a, b)| (a - b).abs() < 1e-15));
    results.push(ValidationResult::check(EXP, "landscape_deterministic", bool_f64(det), 1.0, 0.0));
    ...
    let snap = engagement::EngagementSnapshot {
        ...
        action_count: 50,
        exploration_breadth: session_data.len() as u32,
        challenge_seeking: 3,
        retry_count: 2,
        deliberate_pauses: 5,
    };
    let eng = engagement::compute_engagement(&snap);
    println!("    engagement: apm={:.3}, exploration={:.3}", eng.actions_per_minute, eng.exploration_rate);

    // 5. Engagement valid
    results.push(ValidationResult::check(
        EXP, "engagement_valid", bool_f64(eng.actions_per_minute > 0.0), 1.0, 0.0,
    ));

    // 6. Flow state
    let flow_state = flow::evaluate_flow(0.5, 0.6, 0.15);
    println!("    flow: {:?}", flow_state);
    let in_flow = matches!(flow_state, flow::FlowState::Flow | flow::FlowState::Relaxation);
    results.push(ValidationResult::check(EXP, "exploration_in_flow", bool_f64(in_flow), 1.0, 0.0));
    ...
    results.push(ValidationResult::check(
        EXP, "fun_classified", bool_f64(matches!(fun.dominant, fun_keys::FunKey::Easy)), 1.0, 0.0,
    ));
    ...
    println!("    DDA: {dda:+.3}");
    results.push(ValidationResult::check(EXP, "dda_bounded", bool_f64(dda.abs() < 1.0), 1.0, 0.0));

    // --- Phase 3: Cross-spring validation ---
    println!("\n  Phase 3: Cross-spring validation");

    // 9. W model matches wetSpring Exp356
    let expected_w = 3.5 * 3.0 + 8.0 * 0.1;
    let computed_w = diversity_to_w(3.0, 0.1);
    results.push(ValidationResult::check(EXP, "w_model_matches_wetspring", computed_w, expected_w, 1e-10));

    // 10. Higher W → lower propagation
    let low_w_prop = simulate_qs_propagation(
        &generate_disorder_landscape(32, 32, 5.0, 42), 10.0, 16, 16,
    );
    let high_w_prop = simulate_qs_propagation(
        &generate_disorder_landscape(32, 32, 25.0, 42), 10.0, 16, 16,
    );
    println!("    W=5 propagation: {low_w_prop:.3}, W=25 propagation: {high_w_prop:.3}");
    results.push(ValidationResult::check(
        EXP, "higher_w_lower_propagation", bool_f64(low_w_prop > high_w_prop), 1.0, 0.0,
    ));

    // 11. Five communities → five distinct W values
    let w_values: Vec<f64> = session_data.iter().map(|(_, w, _, _)| *w).collect();
    let all_distinct = w_values.windows(2).all(|pair| (pair[0] - pair[1]).abs() > 0.01);
    results.push(ValidationResult::check(EXP, "communities_distinct_w", bool_f64(all_distinct), 1.0, 0.0));

    // 12. Propagation spans from localized to extended
    let props: Vec<f64> = session_data.iter().map(|(_, _, p, _)| *p).collect();
    let range = props.iter().cloned().reduce(f64::max).unwrap_or(0.0)
        - props.iter().cloned().reduce(f64::min).unwrap_or(0.0);
    println!("    propagation range: {range:.3}");
    results.push(ValidationResult::check(
        EXP, "propagation_range_spans_regimes", bool_f64(range > 0.1), 1.0, 0.0,
    ));
