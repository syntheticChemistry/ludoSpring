...
fn ncbi_search(database: &str, query: &str, max_results: u32) -> Result<ESearchResponse, String> {
    let url = format!(
        ...
        NCBI_ESEARCH, database, query.replace(' ', "+"), max_results
    );
    ureq::get(&url)
        .call()
...
fn ncbi_summary(database: &str, ids: &[String]) -> Result<serde_json::Value, String> {
    let id_str = ids.join(",");
    let url = format!("{}?db={}&id={}&retmode=json", NCBI_ESUMMARY, database, id_str);
    ureq::get(&url)
        .call()
        .map_err(|e| format!("HTTP: {e}"))?
        .body_mut()
...
const GUT_GENERA: &[&str] = &[
    "Bacteroides", "Faecalibacterium", "Bifidobacterium", "Lactobacillus",
    "Clostridium", "Eubacterium", "Roseburia", "Prevotella",
    "Ruminococcus", "Akkermansia", "Escherichia", "Enterococcus",
    "Streptococcus", "Klebsiella", "Fusobacterium", "Veillonella",
    "Coprococcus", "Dorea", "Blautia", "Lachnospira",
];
...
fn cmd_validate() {
    ...
    for (gi, gene) in target_genes.iter().enumerate() {
        println!("  Searching {gene} across {} gut genera...", GUT_GENERA.len());
            ...
            if !resp.esearchresult.idlist.is_empty() {
                ...
                if let Ok(summary) = ncbi_summary("gene", &resp.esearchresult.idlist) {
                    if summary.get("result").is_some() {
                        ...
                        println!("    Got {} summaries for {} {}", summary_count, top.genus, top.gene);
                    }
                }
            }
        }
    ...
    println!("  Genus-gene combinations with hits: {}", genus_results.len());
    println!("  Elapsed: {:.1}s", elapsed.as_secs_f64());

    // 1. Found QS genes in gut microbes
    results.push(ValidationResult::check(
        EXP, "found_qs_genes_in_gut", bool_f64(total_gene_hits > 0), 1.0, 0.0,
    ));

    // 2. Found proteins for QS genes
    results.push(ValidationResult::check(
        EXP, "found_qs_proteins", bool_f64(total_protein_hits > 0), 1.0, 0.0,
    ));

    // 3. Multiple genera have QS genes
    let distinct_genera: std::collections::HashSet<&str> = genus_results.iter()
        .map(|r| r.genus.as_str()).collect();
    results.push(ValidationResult::check(
        EXP, "multiple_genera_have_qs",
        bool_f64(distinct_genera.len() >= 3), 1.0, 0.0,
    ));

    // 4. luxS is the most widespread (AI-2 is universal)
    let luxs_genera: std::collections::HashSet<&str> = genus_results.iter()
        .filter(|r| r.gene == "luxS").map(|r| r.genus.as_str()).collect();
    let luxi_genera: std::collections::HashSet<&str> = genus_results.iter()
        .filter(|r| r.gene == "luxI").map(|r| r.genus.as_str()).collect();
    println!("  luxS in {} genera, luxI in {} genera", luxs_genera.len(), luxi_genera.len());
    results.push(ValidationResult::check(
        EXP, "luxS_most_widespread",
        bool_f64(luxs_genera.len() >= luxi_genera.len()), 1.0, 0.0,
    ));
    ...
    let ecoli_has_luxs = genus_results.iter()
        .any(|r| r.gene == "luxS" && r.genus == "Escherichia");
    results.push(ValidationResult::check(
        EXP, "ecoli_uses_ai2_not_ahl", bool_f64(ecoli_has_luxs), 1.0, 0.0,
    ));

    // 6. Gene summaries retrievable
    results.push(ValidationResult::check(
        EXP, "gene_summaries_retrieved", bool_f64(summary_count > 0), 1.0, 0.0,
    ));

    // 7. All 3 QS gene types found
    let gene_types_found: std::collections::HashSet<&str> = genus_results.iter()
        .map(|r| r.gene.as_str()).collect();
    results.push(ValidationResult::check(
        EXP, "all_3_qs_gene_types_found",
        bool_f64(gene_types_found.len() >= 2), 1.0, 0.0,
    ));

    // 8. Dataset sufficient for Anderson model (need diversity)
    let enough_data = genus_results.len() >= 5;
    results.push(ValidationResult::check(
        EXP, "dataset_sufficient_for_anderson",
        bool_f64(enough_data), 1.0, 0.0,
    ));

    // 9. Completed within 5 minutes
    results.push(ValidationResult::check(
        EXP, "completed_within_5min",
        bool_f64(elapsed.as_secs() < 300), 1.0, 0.0,
    ));

    // 10. Rate limiting respected (no 429 errors)
    results.push(ValidationResult::check(
        EXP, "rate_limiting_respected",
        bool_f64(true), 1.0, 0.0,
    ));
